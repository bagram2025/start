#!/usr/bin/env python3
"""
Админ-панель сервера. Работает за Nginx с префиксом /panel.
"""
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box}
        ...
        .back-btn{color:#eee;text-decoration:none;background:#0f3460;padding:10px 20px;border-radius:8px;font-size:.9em}
        ...
        .card-value{font-size:1.8em;font-weight:bold;color:#e94560;margin-bottom:5px}
        .card-label{color:#888;font-size:.85em}
        .section{margin-bottom:30px}
        .section-title{color:#eee;font-size:1.2em;margin-bottom:15px;padding-bottom:10px;border-bottom:1px solid #0f3460}
        ...
        .chart-bar{height:8px;background:#0f3460;border-radius:4px;margin-top:5px;overflow:hidden}
        .chart-fill{height:100%;border-radius:4px;transition:width 1s}
        .fill-green{background:#4caf50}.fill-yellow{background:#ff9800}.fill-red{background:#e94560}
        @media(max-width:600px){h1{font-size:1.3em}.card-value{font-size:1.3em}}
    </style>
</head>
<body>
    <div class="container">
        ...
        <div class="section">
            ...
            <table><thead><tr><th>PID</th><th>CPU</th><th>MEM</th><th>Процесс</th></tr></thead><tbody id="processes"></tbody></table>
        </div>
        <div class="section">
            <div class="section-title">🔐 Последние входы</div>
            <div id="sshLog" style="font-size:.85em;color:#888;white-space:pre-wrap;"></div>
        </div>
        ...
        async function load() {
            try {
                ...
                document.getElementById('metrics').innerHTML = `
                    ...
                    <div class="card"><div class="card-value">${d.load}</div><div class="card-label">Нагрузка</div></div>
                    <div class="card"><div class="card-value">${d.disk_percent}%</div><div class="card-label">Диск</div></div>
                    <div class="card"><div class="card-value">${d.mem_percent}%</div><div class="card-label">Память</div></div>`;
                ...
                document.getElementById('resources').innerHTML = `
                    <div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between"><span>CPU</span><span>${d.cpu_percent}%</span></div><div class="chart-bar"><div class="chart-fill ${cpuColor}" style="width:${d.cpu_percent}%"></div></div></div>
                    <div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between"><span>Память</span><span>${d.mem_used} / ${d.mem_total}</span></div><div class="chart-bar"><div class="chart-fill ${memColor}" style="width:${d.mem_percent}%"></div></div></div>
                    <div style="margin-bottom:12px"><div style="display:flex;justify-content:space-between"><span>Диск</span><span>${d.disk_used} / ${d.disk_total}</span></div><div class="chart-bar"><div class="chart-fill ${diskColor}" style="width:${d.disk_percent}%"></div></div></div>`;
                document.getElementById('services').innerHTML = d.services.map(s => `<div class="service-row"><span class="service-name">${s.name}</span><span class="service-status ${s.active ? 'status-active' : 'status-inactive'}">${s.active ? 'Active' : 'Inactive'}</span></div>`).join('');
                document.getElementById('processes').innerHTML = d.processes.map(p => `<tr><td>${p.pid}</td><td>${p.cpu}%</td><td>${p.mem}%</td><td style="word-break:break-all">${p.cmd}</td></tr>`).join('');
...
def panel_data():
    ...
    cpu_percent = run_bash("top -bn1 | grep 'Cpu(s)' | awk '{print $2}' | cut -d'%' -f1").replace(",", ".") or "0"
    ...
    services = []
    for svc in ["postgresql", "redis-server", "nginx", "ssh"]:
    ...
    if proc_raw:
        for line in proc_raw.split("\n"):
            parts = line.split()
            if len(parts) >= 4:
                processes.append({"pid": parts[0], "cpu": parts[1], "mem": parts[2], "cmd": " ".join(parts[3:])[:60]})
    ssh_logins = run_bash("last -5 | head -5")
    return jsonify({
